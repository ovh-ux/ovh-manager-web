...
module.exports = (grunt) => {
      ...
      distApp: {
        options: {
          ...
          deps: (() => {
            ...
            _.forEach(assets[target].modules, (module) => {
              if (
                /^ovh-module-([0-9a-zA-Z]+)/.test(module)
              ) {
                const match = module.match(/^ovh-module-([0-9a-zA-Z]+)/)[1];
                deps.push(`Module.${match === 'office' ? 'microsoft' : match}`);
              } else {
                grunt.fail.warn(`Impossible to parse ${module}`);
              }
            });
  ...
  grunt.event.on('watch', (action, filepath, watchName) => {
    ...
    switch (watchName) {
      case 'js':
        ...
        break;
      case 'jsmodules':
        rxCwd = /.+bower_components\/ovh-module-.*\/src\//;
        cwdMatch = filepath.match(rxCwd);
        updateEslint(filepath);
  ...
  grunt.registerTask('wait', function task() {
  ...
  grunt.registerTask('test', (testTarget, option) => {
    let protractorOption = _(option).clone();
    ...
    if (testTarget === 'e2e') {
      ...
      if (
        ...
        protractorOption = 'remote';
      }

      if (protractorOption === 'remote') {
  ...
  grunt.registerTask('build', 'Prod build', () => {
    if (targetsAvailable.indexOf(target) === -1) {
