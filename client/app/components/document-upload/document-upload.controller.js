angular.module("App").controller("DocumentUploadCtrl", [
    ...
    function ($scope, User) {
        ...
        this.uploadFileNameCheck = function (input) {
        ...
        this.uploadFileFileCheck = function (input, file, documentType) {
            input.$setValidity("required", !!file);
            input.$setValidity("format", false);

            if (!documentType || (Array.isArray(documentType) && !!file && file.type && documentType.indexOf(file.type) !== -1)) {
                input.$setValidity("format", true);
            }
        };

        this.isValid = function (file, documentType) {
            ...
            return $scope.formFileUpload.$valid && $scope.formFileUpload.file.$valid && $scope.formFileUpload.file;
        };

        $scope.submit = function () {
            $scope.uploadFile($scope.uploadFileFile, [{ key: "WEB_HOSTING_DATABASE_IMPORT_SCRIPT", value: "true" }], ["application/gzip", "application/x-gzip"]);
        };

        /**
         * @param file: meta of the file
         * @param tags: Array of object that contains key and value ([{key: "DOMAIN_PARAMETER_NAME", value: "X"},...]
         * @param format: Array of allowed file format ex: ['application/gzip',...]
         */
        $scope.uploadFile = function (file, tags, documentType) {
            ...
            User.uploadFile(filename, file, tags)
                .then(
                    (id) => {
                        ...
                        $scope.$emit("DocumentUploadCtrl.onFileUploadSendCompleted", { file, id });
                        $scope.atLeastOneFileHasBeenSend = true;
                    },
                    (err) => {
                        $scope.$emit("DocumentUploadCtrl.onFileUploadSendFailed", { file, error: err });
                    }
                )
                .finally(() => {
