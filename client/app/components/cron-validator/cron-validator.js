angular.module('services').service('CronValidator', function () {
  const self = this;
  ...
  const validacron = {
    ...
    mon: new RegExp(`^(?:(?:\\*|(?:0?\\d|1[0-2]|(?:${months}))(?!\\/)|(?:0?\\d|1[0-2]|(?:${months}))-(?:0?\\d|1[0-2]|(?:${months})))(?:\\/(?:0?\\d|1[0-2])|)(?:,(?!$)|$))+$`, 'i'),
    dow: new RegExp(`^(?:(?:\\*|(?:[0-7]|${days})(?!\\/)|(?:[0-7]|${days})-(?:[0-7]|${days}))(?:\\/(?:[0-7])|)(?:,(?!$)|$))+$`, 'i'),
  };
  ...
  const cronFieldRegex = {
    items: {
      ...
      mon: new RegExp(`^(?:(?:(?:0?\\d|1[0-2]|(?:${months}))|(?:0?\\d|1[0-2]|(?:${months}))-(?:0?\\d|1[0-2]|(?:${months})))(?:,(?!$)|$))+$`, 'i'),
      dow: new RegExp(`^(?:(?:(?:[0-7]|${days})|(?:[0-7]|${days})-(?:[0-7]|${days}))(?:,(?!$)|$))+$`, 'i'),
    },
    interval: {
      h: /^\*\/(0*(?:[2-9]|1\d|2[0-3]))$/,
    ...
    range: {
      ...
      mon: new RegExp(`^(0?\\d|1[0-2]|(?:${months}))-(0?\\d|1[0-2]|(?:${months}))$`, 'i'),
     ...
     */
  this.makeCrontabObject = function () {
    return new self.CrontabObject(self);
  };

  /**
     * @constructor
     */
  this.CrontabObject = function (CronValidator) {
    const cron = new CronValidator.CronValue();
    const mode = new CronValidator.CronMode();

    this.isValid = function () {
      return CronValidator.isCronValid(cron, mode);
    };

    this.setCrontab = function (crontab) {
      CronValidator.initializeFromCrontab(crontab, cron, mode);
    };

    this.getCrontab = function () {
      return CronValidator.toCrontab(cron, mode);
    };
  ...
  this.getSimpleModeItems = function (cronValue, field) {
    let selectValuesTempo = _.pick(cronValue.simple.items[field], selectValue => !!selectValue);
    selectValuesTempo = _.keys(selectValuesTempo);
    return selectValuesTempo.sort((a, b) => a - b);
  };

  // Simple or expert -> crontab
  this.toCrontab = function (cronValue, cronMode) {
  ...
  this.initializeFromCrontab = function (crontab, cronValue, cronMode) {
  ...
  this.switchToSimpleMode = function (cronValue, cronMode) {
  ...
  this.switchToExpertMode = function (cronValue, cronMode) {
  ...
  this.isCronValid = function (cronValue, cronMode) {
    if (cronMode.active === 'simple') {
      // Mode simple
      return self.cronSimpleValueIsValid('h', cronValue, cronMode) && self.cronSimpleValueIsValid('dom', cronValue, cronMode) && self.cronSimpleValueIsValid('mon', cronValue, cronMode) && self.cronSimpleValueIsValid('dow', cronValue, cronMode);
    }

    // Mode expert
    return self.cronExpertValueIsValid('h', cronValue) && self.cronExpertValueIsValid('dom', cronValue) && self.cronExpertValueIsValid('mon', cronValue) && self.cronExpertValueIsValid('dow', cronValue);
  };

  this.cronSimpleValueIsValid = function (field, cronValue, cronMode) {
    switch (cronMode.simple[field]) {
      ...
      case 'range':
        return !!(cronValue.simple.range[field].from != null && cronValue.simple.range[field].to != null && cronValue.simple.range[field].to > cronValue.simple.range[field].from);
      default:
        return false;
    }
  };

  this.cronExpertValueIsValid = function (field, cronValue) {
    return cronValue.expert[field] && validacron[field].test(cronValue.expert[field]);
  };
    ...
    angular.forEach(cron, (val, key) => {
      switch (cronMode.simple[key]) {
        ...
        case 'interval':
          ...
          break;
        case 'range':
          cron[key] = `${cronValue.simple.range[key].from}-${cronValue.simple.range[key].to}`;
          break;
        default:
          break;
      }
  ...
  function expertToCrontab(cronValue) {
    return [cronMinutes, cronValue.expert.h, cronValue.expert.dom, cronValue.expert.mon, cronValue.expert.dow].join(' ');
  }
  ...
  function crontabToSimple(crontab, cronValue, cronMode) {
    ...
    angular.forEach(splittedCrontab, (_val, key) => {
      ...
      if (val === '*') {
        ...
        cronValue.simple.interval[key] = parseInt(val.match(cronFieldRegex.interval[key])[1], 10);
      } else if (cronFieldRegex.range[key].test(val)) {
        // 'N-N' only = range
        val = clearWords(key, val);
        cronMode.simple[key] = 'range';
        cronValue.simple.range[key].from = parseInt(val.match(cronFieldRegex.range[key])[1], 10);
        cronValue.simple.range[key].to = parseInt(val.match(cronFieldRegex.range[key])[2], 10);
        ...
        angular.forEach(splittedItems, (_splittedItem) => {
          ...
          if (cronFieldRegex.range[key].test(splittedItem)) {
            ...
            from = parseInt(splittedItem.match(cronFieldRegex.range[key])[1], 10);
  ...
  function checkIfCrontabIsOkForSimpleMode(crontab) {
    ...
    angular.forEach(splittedCrontab, (val, key) => {
      if (val !== '*' && !cronFieldRegex.interval[key].test(val) && !cronFieldRegex.range[key].test(val) && !cronFieldRegex.items[key].test(val)) {
        isOK = false;
      }
    });
