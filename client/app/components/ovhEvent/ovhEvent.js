angular.module('directives').directive('ovhEvent', () => ({
  ...
  link($scope, $elm, $attrs) {
    if (!!$attrs.ovhEvent && !!$attrs.ovhEventCallback && typeof $scope[$attrs.ovhEventCallback] === 'function') {
      if ($attrs.ovhEventTarget) {
        if ($attrs.ovhEventData) {
          $($elm).delegate($attrs.ovhEventTarget, $attrs.ovhEvent, $attrs.ovhEventData, (evt) => {
        ...
        angular.element($elm).bind($attrs.ovhEvent, $attrs.ovhEventData, (evt) => {
          $scope[$attrs.ovhEventCallback]($attrs.ovhEventData, evt);
        });
      } else {
