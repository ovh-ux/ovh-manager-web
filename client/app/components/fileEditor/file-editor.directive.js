    ...
    function ($anchorScroll, $location) {
        ...
        return {
            ...
            link: ($scope, element) => {
                ...
                $scope.fileEditorMatchExistingItems = angular.isDefined($scope.fileEditorMatchExistingItems) ? $scope.fileEditorMatchExistingItems : false;
                ...
                $scope.goToNextError = () => {
                    const nextError = _.findIndex($scope.ngModel, (item) => !$scope.fileEditorItemValidator(item));
                    if (nextError !== -1) {
                ...
                $scope.$watch("fileModel.value", () => {
                    ...
                    if ($scope.fileModel.value) {
                        ...
                        if (!$scope.fileEditorMatchExistingItems) {
                            $scope.ngModel = $scope.ngModel.filter((item) => !!item && $scope.fileEditorExistingItems.indexOf(item) === -1);
                        } else {
                            $scope.ngModel = $scope.ngModel.filter((item) => $scope.fileEditorExistingItems.indexOf(item) !== -1);
                        }
                    } else {
                        $scope.ngModel = [];
                    }
                ...
                $scope.$watch(
                    ...
                    () => {
                        if (Array.isArray($scope.ngModel)) {
                            ...
                            $scope.fileEditorErrors = $scope.ngModel.filter((item) => !$scope.fileEditorItemValidator(item));
                        }
                    },
                    true
                );
