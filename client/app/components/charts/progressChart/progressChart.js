                ...
                (nv) => {
                    if (nv !== undefined) {
                        if (nv < $scope.min) {
                            ...
                            $scope.progressGreen = computePercent($scope.yellow, $scope.min, $scope.max);

                            if (nv < $scope.red) {
                                $scope.progressYellow = computePercent(nv, $scope.min, $scope.max) - $scope.progressGreen;
                                $scope.progressRed = 0;
                            } else {
                                $scope.progressYellow = computePercent($scope.red, $scope.min, $scope.max) - $scope.progressGreen;
                                if (nv < $scope.max) {
                                    $scope.progressRed = computePercent(nv, $scope.min, $scope.max) - $scope.progressGreen - $scope.progressYellow;
                                } else {
                                    $scope.progressRed = computePercent($scope.max, $scope.min, $scope.max) - $scope.progressGreen - $scope.progressYellow;
                                }
                            }
                        }
                    }
