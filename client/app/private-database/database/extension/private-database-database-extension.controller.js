  ...
  .controller('PrivateDatabaseBDDsExtensionCtrl', class PrivateDatabaseBDDsExtensionCtrl {
    ...
    getExtensions() {
      ...
      this.privateDatabaseExtensionService.getExtensions(this.productId, this.bdd.databaseName)
        .then((extensions) => {
          this.extensions = extensions.sort().map(id => ({ id, extensionName: id }));
        })
        .catch((err) => {
    ...
    toggleExtension(extension) {
      if (!extension.updating && extension.transformed) {
        ...
        if (extension.enabled) {
          action = this.enableExtension(extension)
            .then(() => this.alerter.success(this.$scope.tr('privateDatabase_tabs_list_extensions_enable_success'), this.$scope.alerts.main));
        } else {
          action = this.disableExtension(extension)
            .then(() => this.alerter.success(this.$scope.tr('privateDatabase_tabs_list_extensions_disable_success'), this.$scope.alerts.main));
        }
        action.catch((error) => {
    ...
    transformItem(item, force) {
      ...
      return this.privateDatabaseExtensionService.getExtension(this.productId, this.bdd.databaseName, item.id)
        .then((extension) => {
