angular
  .module('services')
  .service('PrivateDatabaseExtension', class PrivateDatabaseExtension {
    ...
    getExtensions(serviceName, databaseName, extensionName, status) {
      return this.$http.get(`apiv6/hosting/privateDatabase/${serviceName}/database/${databaseName}/extension`, {
        params: {
          extensionName,
          status,
        },
    ...
    getExtension(serviceName, databaseName, extensionName) {
      return this.$http.get(`apiv6/hosting/privateDatabase/${serviceName}/database/${databaseName}/extension/${extensionName}`)
        .then(res => res.data);
    }
    ...
    disableExtension(serviceName, databaseName, extensionName) {
      return this.$http.post(`apiv6/hosting/privateDatabase/${serviceName}/database/${databaseName}/extension/${extensionName}/disable`)
        .then((task) => {
    ...
    pollExtensionDisable(serviceName, opts) {
      ...
      return this.privateDatabaseService.poll(serviceName, {
        taskId: opts.taskId,
        namespace,
    ...
    extensionDisableSuccess(serviceName, opts) {
      return this.getExtensions(serviceName, opts.databaseName, opts.extensionName)
        .then((extensions) => {
          const options = angular.copy(opts);
    ...
    killPollExtensionDisable() {
      this.pollService.kill({ namespace: 'privateDatabase.database.extension.delete' });
    }
    ...
    enableExtension(serviceName, databaseName, extensionName) {
      return this.$http.post(`apiv6/hosting/privateDatabase/${serviceName}/database/${databaseName}/extension/${extensionName}/enable`)
        .then((task) => {
    ...
    killPollExtensionEnable() {
      this.pollService.kill({ namespace: 'privateDatabase.database.extension.create' });
    }
  });
