angular
    ...
    .controller("PrivateDatabaseStateCtrl", class PrivateDatabaseStateCtrl {
        ...
        getOomList () {
            return this.oomService.getOomList(this.productId)
                .then((oomList) => {
        ...
        getRealList () {
            this.database.oom.realList = _.filter(this.database.oom.list, (item) => filesize(item.sizeReached, { output: "object" }).value > this.database.ram.value);
            return this.database.oom.realList;
        }

        getHostingsLinked () {
            this.privateDatabaseService.getHostingsLinked(this.productId)
                .then((hostingsLinkedsId) =>
                    this.$q.all(
                        _.map(hostingsLinkedsId, (mutuName) =>
                ...
                .then((hostingsLinked) => (this.hostingsLinked = hostingsLinked))
        ...
        isAdminMutu (mutu) {
            this.getUserInfos()
                ...
                .then((mutuInfo) => _.some([mutuInfo.contactBilling, mutuInfo.contactTech, mutuInfo.contactAdmin], (contactName) => this.userInfos.nichandle === contactName))
                .catch((err) => {
        ...
        changeRootPassword () {
            ...
            this.$scope.setAction("user/update/password/root/private-database-user-update-password-root", {
