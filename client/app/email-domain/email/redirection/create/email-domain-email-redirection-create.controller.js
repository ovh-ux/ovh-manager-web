angular.module("App").controller(
    ...
    class EmailsCreateRedirectionCtrl {
        ...
        isAccountNameValid (name) {
            return !name || (name.length >= this.constants.nameMinLength && name.length <= this.constants.nameMaxLength && this.constants.nameRegexPattern.test(name));
        }

        redirectionToCheck (input) {
            input.$setValidity("redirectionTo", validator.isEmail(this.model.redirectionTo));
        }

        createRedirection () {
            return this.Emails
                .createRedirection(this.$stateParams.productId, {
                ...
                .then(() => this.Alerter.success(this.$scope.tr("email_tab_modal_create_redirection_success"), this.$scope.alerts.emailDomainRedirects))
                .catch((err) => this.Alerter.alertFromSWS(this.$scope.tr("email_tab_modal_create_redirection_error"), err, this.$scope.alerts.emailDomainRedirects))
