        ...
        if (!$scope.loaders.versions) {
            ...
            Hosting.getModels()
                .then(
                    ...
                    (data) => {
                        Alerter.alertFromSWS($scope.tr("hosting_dashboard_database_versions_error", [$scope.entryToDelete]), data.data, $scope.alerts.dashboard);
                    }
                )
                .finally(() => {
            ...
            HostingDatabase.activeDatabasePrivate($stateParams.productId, $scope.choice.ram.quota.value, $scope.choice.version)
                .then(
                    () => {
                        ...
                        Alerter.success($scope.tr("hosting_dashboard_database_active_success"), $scope.alerts.dashboard);
                    },
                    (data) => {
                        Alerter.alertFromSWS($scope.tr("hosting_dashboard_database_active_error", [$scope.entryToDelete]), data.data, $scope.alerts.dashboard);
                    }
                )
                .finally(() => {
                    $scope.resetAction();
