    ...
    class HostingModuleDeleteCtrl {
        ...
        deleteModule () {
            ...
            return this.HostingModule.deleteModule(this.$stateParams.productId, this.moduleToDelete.id)
                .then(() => {
                    this.Alerter.success(this.$scope.tr("hosting_configuration_tab_modules_delete_success"), this.$scope.alerts.modules);
                })
                .catch((err) => {
                    this.Alerter.alertFromSWS(this.$scope.tr("hosting_configuration_tab_modules_delete_fail", [this.moduleToDelete]), err, this.$scope.alerts.modules);
                });
        }
    }
);
