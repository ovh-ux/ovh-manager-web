angular
    ...
    .controller("HostingCtrl", ($scope, $rootScope, $q, $timeout, ConverterService, Hosting, Screenshot, Alerter, Navigator, constants, User, HostingOvhConfig, HostingTask, HostingDatabase, HostingDomain, PrivateDatabase, $stateParams) => {
        ...
        $scope.convertBytesSize = (nb, unit, decimalWanted = 0) => {
            const res = filesize(ConverterService.convertToOctet(nb, unit), { output: "object", round: decimalWanted, base: -1 });
        ...
        function checkSqlPriveState () {
            ...
            Hosting.checkTaskUnique($stateParams.productId, "hosting/activate/privateDatabase").then((taskIds) => {
                if (taskIds && taskIds.length) {
        ...
        $scope.getOfferPrivateSQLInfo = function () {
            ...
            HostingDatabase.getPrivateDatabaseCapabilities($stateParams.productId).then((privateDbCapabilities) => {
                $scope.hosting.sqlPriveInfo.nbDataBaseInclude = $scope.hosting.offerCapabilities.privateDatabases.length;
                $scope.hosting.sqlPriveInfo.nbDataBaseActive = $scope.hosting.sqlPriveInfo.nbDataBaseInclude - privateDbCapabilities.length;
            });
        };
        ...
        function startPolling () {
            $q.all([HostingDomain.getTaskIds({ fn: "attachedDomain/create" }, $stateParams.productId), HostingDomain.getTaskIds({ fn: "attachedDomain/update" }, $stateParams.productId)]).then((tasks) => {
