angular.module("App").controller("HostingTabCronsCtrl", ($scope, Hosting, $timeout, $stateParams, HostingCron, Alerter, User) => {
    "use strict";
    ...
    $scope.loadCrons = () => {
        ...
        if ($scope.search.text) {
            filters = [{ command: ["%", $scope.search.text, "%"].join("") }, { description: ["%", $scope.search.text, "%"].join("") }, { email: ["%", $scope.search.text, "%"].join("") }];
        }
