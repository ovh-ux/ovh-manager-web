    ...
    class HostingFtpUserDeleteCtrl {
        ...
        deleteUser () {
            ...
            return this.HostingUser.deleteUser(this.$stateParams.productId, this.entryToDelete)
                .then(() => {
                    this.Alerter.success(this.$scope.tr("hosting_tab_DATABASES_configuration_user_delete_success"), this.$scope.alerts.ftp);
                })
                .catch((err) => {
                    this.Alerter.alertFromSWS(this.$scope.tr("hosting_tab_DATABASES_configuration_user_delete_fail", [this.entryToDelete]), _.get(err, "data", err), this.$scope.alerts.ftp);
                });
        }
    }
);
