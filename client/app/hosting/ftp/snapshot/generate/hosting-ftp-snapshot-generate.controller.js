    ...
    class HostingFtpGenerateSnapshotCtrl {
        ...
        $onInit () {
            ...
            this.HostingFtp.getModels()
                .then((data) => {
                    ...
                    this.Hosting.getSelected(this.$stateParams.productId)
                        .then((hosting) => { // remove backup snapshots that are not yet available
                            if (moment(hosting.creation).isAfter(moment().subtract(1, "weeks"))) {
                ...
                .catch((err) => {
                    this.Alerter.alertFromSWS(this.$scope.tr("hosting_tab_FTP_configuration_generate_snapshot_error"), err, this.$scope.alerts.main);
                })
                .finally(() => {
                    this.loading = false;
                })
        ...
        generateSnapshot () {
            ...
            return this.HostingFtp.restoreSnapshot(this.$stateParams.productId, { backup: this.model.snapshotInstance })
                .then(() => {
                    this.Alerter.success(this.$scope.tr("hosting_tab_FTP_configuration_generate_snapshot_success"), this.$scope.alerts.main);
                    this.$rootScope.$broadcast("hosting.tabs.tasks.refresh");
                })
                .catch((err) => {
                    this.Alerter.alertFromSWS(this.$scope.tr("hosting_tab_FTP_configuration_generate_snapshot_error"), _.get(err, "data", err), this.$scope.alerts.main);
                })
                .finally(() => {
                    this.loading = false;
