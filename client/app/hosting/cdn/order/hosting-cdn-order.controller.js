angular.module('App').controller('HostingOrderCdnCtrl', ($scope, $q, $window, HostingOptionOrder, Hosting, Alerter, $rootScope) => {
  $scope.loading = {
    init: true,
    duration: true,
    model: false,
  ...
  $scope.loadOrder = () => {
    ...
    HostingOptionOrder.isOptionOrderable('cdn').then((isOrderable) => {
      ...
      HostingOptionOrder.getOrderEnums('hosting.web.CdnOfferEnum').then((models) => {
        ...
        $scope.model.offer = $scope.availableOffers[0];
    ...
    HostingOptionOrder.getOrderDurations('cdn', { offer: $scope.model.offer })
      .then((durations) => {
        ...
        angular.forEach($scope.durations, (duration) => {
          queue.push(HostingOptionOrder.getOrderDetailsForDuration('cdn', duration, { offer: $scope.model.offer }).then((details) => {
