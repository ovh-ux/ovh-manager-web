angular.module("App").controller(
    ...
    class DomainOperationUpdateCtrl {
        ...
        $onInit () {
            ...
            this.baseArgs = [];
            this.canContinue = this.operation.canAccelerate || this.operation.canCancel || this.operation.canRelaunch;
            this.constraints = {};
            this.document = false;
            this.documents = {};
            this.files = {};
            ...
            return this.domainOperationService
                ...
                .then((argumentIds) => {
                    const promises = _.map(argumentIds, (key) =>
                        this.domainOperationService.getOperationArgument(operationId, key).then((argument) => {
                            this.document = this.document || argument.type === "/me/document";

                            // add user friendly translations for some known tasks
                            if (_.indexOf(["action", "memberContactXXX"], argument.key) !== -1) {
                                argument.keyTranslation = this.$scope.tr(`domains_operations_update_key_${argument.key}`);
                            }

                            // set a default value
                            if (_.isArray(argument.acceptedValues) && argument.acceptedValues.length > 1) {
                                argument.value = argument.acceptedValues[0];
                            }
                            ...
                            if (argument.type === "/me/document") {
                                ...
                                if (argument.acceptedFormats) {
                                    this.constraints[argument.key].acceptedFormatsDisplay = argument.acceptedFormats.join(", ");
                                    this.constraints[argument.key].acceptedFormats = _.chain(argument.acceptedFormats).map((value) => `.${value}`).join(", ").value();
                                } else {
        ...
        upload (file, arg) {
            if (!!this.constraints[arg.key].maximumSize && file.size > this.constraints[arg.key].maximumSize) {
            ...
            const promises = _.map(this.args, (arg) => {
                ...
                if (arg.type === "/me/document") {
                    if (this.files[arg.key]) {
                        rtn = this.User.uploadFile(arg.key, this.files[arg.key]).then((documentId) => this.domainOperationService.updateOperation({ id: this.operation.id, key: arg.key, data: { value: documentId } }));
                    }
                }
                if (!arg.readOnly) {
                    rtn = this.domainOperationService.updateOperation({ id: this.operation.id, key: arg.key, data: { value: arg.value } });
                }
                return rtn;
            });
