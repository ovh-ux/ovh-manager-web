angular.module("App").controller(
    ...
    class DomainDynHostEditCtrl {
        ...
        updateDynHost () {
            this.loading = true;
            return this.Domain
                .updateDynHost(this.product.name, this.dynHost.id, {
                    ip: this.dynHost.ip,
                    subDomain: punycode.toASCII(this.dynHost.subDomain)
                })
                .then(() => this.Domain.refreshZoneState(this.product.name).then(() => this.Alerter.success(this.$scope.tr("domain_tab_DYNHOST_edit_success"), this.$scope.alerts.domainDynhost)))
                .catch((err) => this.Alerter.alertFromSWS(this.$scope.tr("domain_tab_DYNHOST_error"), _.get(err, "data", err), this.$scope.alerts.domainDynhost))
                .finally(() => {
                    this.loading = false;
