        ...
        unlockDomain () {
            return this.Domain
                ...
                .then((data) => {
                    ...
                    this.Alerter.success(this.$scope.tr("domain_configuration_protection_desactivate_success"), this.$scope.alerts.dashboard);
                })
                .catch((err) => {
                    this.$rootScope.$broadcast("domain.protection.unlock.error", err);
                    this.Alerter.alertFromSWS(this.$scope.tr("domain_configuration_protection_desactivate_fail"), _.get(err, "data", err), this.$scope.alerts.dashboard);
                })
                .finally(() => {
                    this.$scope.resetAction();
                });
