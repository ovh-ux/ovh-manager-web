    ...
    class DomainRedirectionAddCtrl {
        ...
        $onInit () {
            ...
            this.newRedirection = {
                ...
                subdomain: _.get(this.$scope.currentActionData, "subdomainPreset", null),
                subdomainWww: "www",
            ...
            this.typeRedirection = { txt: "TXT", cname: "CNAME", ipv4: "A", ipv6: "AAAA" };
        ...
        getCompleteTarget () {
            ...
            if (!_.isEmpty(redirectionTarget) && this.shouldIncludeDomain && _.isString(this.newRedirection.domain.name)) {
        ...
        getDomainNameOf (subDomain) {
            return (subDomain !== null && subDomain !== "" ? `${punycode.toUnicode(subDomain)}.` : "") + this.newRedirection.domain.displayName;
        }
        ...
        getSmallDomainNameOf () {
            if (this.errors.containOneError && !this.errors.containAllError && this.errors.wwwhasError) {
                return this.getDomainNameOf(this.newRedirection.subdomain);
            }
            if (this.newRedirection.addwww && !this.newRedirection.disableWww && this.errors.containOneError && !this.errors.containAllError && !this.errors.wwwhasError) {
                return this.getDomainNameOf(this.newRedirection.subdomainWww);
            }
            return ((this.newRedirection.addwww && !this.newRedirection.disableWww && this.$scope.i18n.domain_tab_REDIRECTION_add_www) || "") + this.getDomainNameOf(this.newRedirection.subdomain);
        }
         ...
         */
        getLastError (table) {
            const record = _.findLast(table, (r) => !this.constructor.isValidRedirection(r));
            if (record) {
         ...
         */
        getTitle () {
            return [this.getDomainNameOf(this.newRedirection.subdomain), (this.newRedirection.addwww && !this.newRedirection.disableWww && this.$scope.tr("domain_and", this.getDomainNameOf(this.newRedirection.subdomainWww))) || ""];
        }
        ...
        testInputSize (val) {
            ...
            const subDomainLength = this.newRedirection.subdomain ? punySubDomain.length : 0;
        ...
        redirectionTargetChange () {
            if (_.isEmpty(this.newRedirection.serverTarget)) {
                ...
                this.errors.redirectionTarget = !this.Validator.isDomainNameValid(domainName);
         ...
         */
        ipaddrValid (ip) {
            if (this.Validator.isIPv4Valid(ip)) {
                return this.typeRedirection.ipv4;
            } else if (this.Validator.isIPv6Valid(ip)) {
        ...
        static isValidRedirection (redirection) {
            return redirection.state === "OK" && _.isEmpty(redirection.listBlockingType);
        }

        subDomainCheck (input) {
            const subDomain = this.newRedirection.subdomain;
            const isValid = subDomain === null || subDomain === "" || this.DomainValidator.isValidTarget(`${subDomain}.${this.newRedirection.domain.displayName}`, "CNAME");
            input.$setValidity("subdomain", isValid);

            if (subDomain !== null && subDomain !== "") {
                this.newRedirection.disableWww = subDomain === "www" || _.startsWith(subDomain, "www.");
                this.newRedirection.subdomainWww = !this.newRedirection.disableWww ? `www.${subDomain}` : "www";
            } else {
        ...
        webTargetCheck () {
            ...
            if (webTarget) {
                this.errors.webTarget = !this.Validator.isURLValid(webTarget);
        ...
        webTargetDescriptionCheck () {
            this.errors.ortDescription = this.testInputSize(this.newRedirection.webTargetdetail.webTargetDescription);
        }
        ...
        webTargetKeywordsCheck () {
            this.errors.ortKeywords = this.testInputSize(this.newRedirection.webTargetdetail.webTargetKeywords);
        }
        ...
        step4Validator () {
            if (this.newRedirection.step2 === this.choice.web) {
                if (this.newRedirection.step3 === this.choice.visible) {
                    if (this.newRedirection.webTarget && !this.errors.webTarget && !this.errors.webTargetLength && this.newRedirection.step4 === this.choice.r301) {
                        ...
                        return this.Validator.isURLValid(this.newRedirection.params.targetRedirection);
                    }
                ...
                if (this.newRedirection.step3 === this.choice.ip) {
                    const validIP = this.newRedirection.ipTarget && this.ipaddrValid(this.newRedirection.ipTarget);
                    if (validIP) {
        ...
        checkValidity () {
            ...
            this.Domain
                .checkRedirectionAdd(this.newRedirection.domain.name, {
                    params: _.extend(this.newRedirection.params, { subDomain }),
                    considerWww: this.newRedirection.addwww && !this.newRedirection.disableWww
                })
                .then((ids) => {
        ...
        addRedirection () {
            ...
            this.Domain
                .overwriteRedirection(
                    ...
                    {
                        params: _.extend(this.newRedirection.params, { subDomain, detailORT: this.newRedirection.webTargetdetail }),
                        considerWww: this.newRedirection.addwww && !this.newRedirection.disableWww
                    },
                    this.errors.conflict || this.errors.containAllError || this.errors.containOneError ? this.newRedirection.listValidRedirection : null
                )
                .then((tab) => {
                    let nbError = 0;
                    _.forEach(tab, (item) => {
                    ...
                    if (nbError === tab.length) {
                        this.Alerter.error(this.$scope.tr("domain_tab_REDIRECTION_add_error"), this.$scope.alerts.dashboard);
                    } else if (nbError !== 0) {
                        this.Alerter.alertFromSWS(this.$scope.tr("domain_tab_REDIRECTION_add_partial"), "PARTIAL", this.$scope.alerts.dashboard);
                    } else {
                        this.Alerter.success(this.$scope.tr("domain_tab_REDIRECTION_add_success"), this.$scope.alerts.dashboard);
                    }
                })
                .catch((err) => {
                    this.Alerter.alertFromSWS(this.$scope.tr("domain_tab_REDIRECTION_delete_error"), err, this.$scope.alerts.dashboard);
                })
                .finally(() => {
