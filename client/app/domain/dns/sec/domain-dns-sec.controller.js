    ...
    class DomainDnsSecCtrl {
        ...
        changeDnsSecState () {
            ...
            return this.DomainsDnsSec
                ...
                .then((data) => {
                    if (data.state !== "OK") {
                        this.Alerter.alertFromSWS(this.$scope.tr(`domain_configuration_dnssec_error_${newState}`), data, this.$scope.alerts.domainDns);
                    }
                })
                .catch((err) => this.Alerter.alertFromSWS(this.$scope.tr(`domain_configuration_dnssec_error_${newState}`), _.get(err, "data", err), this.$scope.alerts.domainDns))
                .finally(() => {
